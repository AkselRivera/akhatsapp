...
window.addEventListener('load', (event) => {
  ...
  conn = new WebSocket(
    'ws://' + document.location.host + `/ws?nickname=${NICKNAME}`
  )
  ...
  conn.onmessage = function (event) {
    ...
    appendMessage(message.nickname, 'incoming', message.content)
  }
})
...
chatForm.addEventListener('submit', (event) => {
  ...
  appendMessage(NICKNAME, 'outgoing', message)
  chatInput.value = ''
...
function appendMessage(nickname, type, message) {
  ...
  const doScroll =
    chatTalk.scrollTop > chatTalk.scrollHeight - chatTalk.clientHeight - 1
